...
class Booking(models.Model):
    ...
    # Поле для назначенного времени консультации (заполняется экспертом при подтверждении)
    ...
    # Дополнительные поля, заполняемые пользователем при запросе
    ...
    available_times = models.TextField(
        verbose_name="Предпочтительные временные интервалы", 
        blank=True, 
        null=True,
