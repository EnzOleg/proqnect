...
def register_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request, f"Ошибка регистрации: {form.errors}")
...
# =================================================
    #   Профиль, посты. (когда нибудь и друзья)
# =================================================
@login_required
def profile_view(request):
    # Получаем посты пользователя, сортируя их по убыванию даты
    posts = request.user.posts.all().order_by("-created_at")
    # Здесь можно добавить логику для получения друзей. Пока передадим пустой список.
    friends = []  
